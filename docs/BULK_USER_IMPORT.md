...
招待メールを送らずに、CSVから「メール確認済み・利用規約同意済み・ニュースレター受信OFF」のログイン可能なアカウントを一括作成する仕組みです。
...
Excel で保存した CSV（UTF-8 BOM 付き）もそのまま読み込めます。文字コードは UTF-8 にしてください。
...
- `failed`: バリデーションエラーなど。1行の失敗が他の行の処理を止めることはありません
