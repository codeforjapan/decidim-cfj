...
- 同期処理のため、**1回あたり 100行・1MB** が上限です。超える場合はCSVを分割するか、rake タスクを使ってください
  - 本番の CDN・ロードバランサのタイムアウトに収まる件数にしています（具体値はインフラ側の設定を参照）
...
- 管理画面からの実行は同期処理のため、1回あたり 100行・1MB が上限です（大量件数は rake タスクを使うか、ActiveJob 化を検討してください）
