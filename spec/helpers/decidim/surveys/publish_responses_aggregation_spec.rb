...
# aggregates with `question.responses.map { ... }.tally`, which counts *combinations*
...
module Decidim
  module Surveys
    describe PublishResponsesHelper do
      ...
      let!(:option_a) { create(:response_option, question:, body: { "en" => "Option A", "ja" => "選択肢A" }) }
      let!(:option_b) { create(:response_option, question:, body: { "en" => "Option B", "ja" => "選択肢B" }) }
      let!(:option_c) { create(:response_option, question:, body: { "en" => "Option C", "ja" => "選択肢C" }) }
      ...
      before do
        ...
        [option_a, option_b, option_c].each do |opt|
          create(:response_choice, response: response1, response_option: opt, matrix_row: nil, body: opt.body["en"])
        end

        # Answer 2: selects only A
        response2 = create(:response, questionnaire:, question:)
        create(:response_choice, response: response2, response_option: option_a, matrix_row: nil, body: option_a.body["en"])

        # Answer 3: selects only C
        response3 = create(:response, questionnaire:, question:)
        create(:response_choice, response: response3, response_option: option_c, matrix_row: nil, body: option_c.body["en"])
      end
      ...
      describe "#chart_for_question aggregation for single_option" do
        ...
        let!(:s_opt_x) { create(:response_option, question: single_question, body: { "en" => "X", "ja" => "X" }) }
        let!(:s_opt_y) { create(:response_option, question: single_question, body: { "en" => "Y", "ja" => "Y" }) }

        before do
          # 2 answers pick X, 1 answer picks Y
          [s_opt_x, s_opt_x, s_opt_y].each do |opt|
            a = create(:response, questionnaire:, question: single_question)
            create(:response_choice, response: a, response_option: opt, matrix_row: nil, body: opt.body["en"])
          end
        end
