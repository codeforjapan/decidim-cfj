...
RSpec.describe Decidim::BulkSpaceAccountIssuer do
  subject(:issuer) { described_class.new(organization:, email_domain: "example.test") }
  ...
  describe "#issue" do
    context "with participants" do
      ...
      it "creates accounts with zero-padded sequential ids" do
        ...
        expect(results.map(&:email)).to eq(%w(a-high-001@example.test a-high-002@example.test))
      end
    ...
    context "with dry_run" do
      subject(:issuer) { described_class.new(organization:, email_domain: "example.test", dry_run: true) }
    ...
    context "when the generated email already exists with a different nickname" do
      before { create(:user, organization:, email: "a-high-001@example.test", nickname: "someone-else") }
  ...
  describe "#initialize" do
    ...
    it "rejects a domain without a dot" do
      expect { described_class.new(organization:, email_domain: "example") }
