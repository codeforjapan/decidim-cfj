...
module Decidim
  ...
  module LayoutHelper
    ...
    def favicon
      ...
      if current_organization.favicon.respond_to?(:attached?) && current_organization.favicon.attached?
        safe_join(Decidim::OrganizationFaviconUploader::SIZES.map do |version, size|
          image =
            if version == :favicon && current_organization.favicon.content_type != "image/vnd.microsoft.icon"
              current_organization.favicon.variant(resize: "#{size}x#{size}!")
            else
          ...
          favicon_link_tag(icon_image, sizes: "#{size}x#{size}")
        end)
      else
        safe_join(Decidim::OrganizationFaviconUploader::SIZES.map do |version, size|
          favicon_link_tag(
            current_organization.attached_uploader(:favicon).variant_url(version, host: current_organization.host),
            sizes: "#{size}x#{size}"
          )
        end)
      end
    ...
    def apple_favicon
      if current_organization.favicon.respond_to?(:attached?) && current_organization.favicon.attached?
        image = current_organization.favicon.variant(resize: "180x180!")
        icon_image = Rails.application.routes.url_helpers.rails_representation_path(image, only_path: true)
      else
